"use client";

import { useRef } from "react";
import Image from "next/image";
import Link from "next/link";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import { useGSAP } from "@gsap/react";

if (typeof window !== "undefined") {
  gsap.registerPlugin(ScrollTrigger);
}

interface StatBlock {
  stat: string;
  explainer: string;
  imageSrcDesktop: string;
  imageSrcMobile: string;
  link: string;
}

const STATS_DATA: StatBlock[] = [
  {
    stat: "70%",
    explainer: "Our modular approach speeds up home construction by up to 70% compared to traditional methods.",
    imageSrcDesktop: "https://www.kingshaus.co/_nuxt/image/e0b2f7.jpeg",
    imageSrcMobile: "https://www.kingshaus.co/_nuxt/image/27a463.jpeg",
    link: "/build-smarter/"
  },
  {
    stat: "40%",
    explainer: "Our homes use 40% less energy for heating or cooling than traditional brick or concrete houses.",
    imageSrcDesktop: "https://www.kingshaus.co/_nuxt/image/51010e.jpeg",
    imageSrcMobile: "https://www.kingshaus.co/_nuxt/image/cd06cd.jpeg",
    link: "/build-smarter/"
  }
];

// SVG Arrow component for reuse in both layers
const ArrowIcon = () => (
  <svg width="90" height="90" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg" className="interactive-stat__arrow">
    <circle cx="45" cy="45" r="45" fill="white"></circle>
    <path d="M41 27L49 27L49 35" stroke="#354D43" strokeWidth="2"></path>
    <path d="M41.6956 34.1304L49 26.9999" stroke="#354D43" strokeWidth="2"></path>
    <path d="M32.8163 45.632C33.2803 45.784 33.6483 46.04 33.9203 46.4C34.1923 46.752 34.3283 47.192 34.3283 47.72C34.3283 48.448 34.0483 49.012 33.4883 49.412C32.9283 49.804 32.1123 50 31.0403 50H26.8643V41.6H30.8003C31.7923 41.6 32.5563 41.796 33.0923 42.188C33.6283 42.572 33.8963 43.104 33.8963 43.784C33.8963 44.2 33.8003 44.568 33.6083 44.888C33.4163 45.208 33.1523 45.456 32.8163 45.632ZM28.4243 42.824V45.14H30.6323C31.1763 45.14 31.5923 45.044 31.8803 44.852C32.1763 44.652 32.3243 44.364 32.3243 43.988C32.3243 43.604 32.1763 43.316 31.8803 43.124C31.5923 42.924 31.1763 42.824 30.6323 42.824H28.4243ZM30.9443 48.776C32.1523 48.776 32.7563 48.372 32.7563 47.564C32.7563 46.756 32.1523 46.352 30.9443 46.352H28.4243V48.776H30.9443ZM39.6052 50.12C38.4452 50.12 37.5412 49.796 36.8932 49.148C36.2452 48.492 35.9212 47.556 35.9212 46.34V41.6H37.4812V46.28C37.4812 47.928 38.1932 48.752 39.6172 48.752C41.0332 48.752 41.7412 47.928 41.7412 46.28V41.6H43.2772V46.34C43.2772 47.556 42.9532 48.492 42.3052 49.148C41.6652 49.796 40.7652 50.12 39.6052 50.12ZM45.462 41.6H47.022V50H45.462V41.6ZM49.294 41.6H50.854V48.68H55.246V50H49.294V41.6ZM56.4776 41.6H60.1496C61.0456 41.6 61.8416 41.776 62.5376 42.128C63.2336 42.472 63.7736 42.964 64.1576 43.604C64.5416 44.236 64.7336 44.968 64.7336 45.8C64.7336 46.632 64.5416 47.368 64.1576 48.008C63.7736 48.64 63.2336 49.132 62.5376 49.484C61.8416 49.828 61.0456 50 60.1496 50H56.4776V41.6ZM60.0776 48.68C60.6936 48.68 61.2336 48.564 61.6976 48.332C62.1696 48.092 62.5296 47.756 62.7776 47.324C63.0336 46.884 63.1616 46.376 63.1616 45.8C63.1616 45.224 63.0336 44.72 62.7776 44.288C62.5296 43.848 62.1696 43.512 61.6976 43.28C61.2336 43.04 60.6936 42.92 60.0776 42.92H58.0376V48.68H60.0776ZM18.8863 62.12C18.2383 62.12 17.6103 62.028 17.0023 61.844C16.4023 61.66 15.9263 61.416 15.5743 61.112L16.1143 59.9C16.4583 60.172 16.8783 60.396 17.3743 60.572C17.8783 60.74 18.3823 60.824 18.8863 60.824C19.5103 60.824 19.9743 60.724 20.2783 60.524C20.5903 60.324 20.7463 60.06 20.7463 59.732C20.7463 59.492 20.6583 59.296 20.4823 59.144C20.3143 58.984 20.0983 58.86 19.8343 58.772C19.5703 58.684 19.2103 58.584 18.7543 58.472C18.1143 58.32 17.5943 58.168 17.1943 58.016C16.8023 57.864 16.4623 57.628 16.1743 57.308C15.8943 56.98 15.7543 56.54 15.7543 55.988C15.7543 55.524 15.8783 55.104 16.1263 54.728C16.3823 54.344 16.7623 54.04 17.2663 53.816C17.7783 53.592 18.4023 53.48 19.1383 53.48C19.6503 53.48 20.1543 53.544 20.6503 53.672C21.1463 53.8 21.5743 53.984 21.9343 54.224L21.4423 55.436C21.0743 55.22 20.6903 55.056 20.2903 54.944C19.8903 54.832 19.5023 54.776 19.1263 54.776C18.5103 54.776 18.0503 54.88 17.7463 55.088C17.4503 55.296 17.3023 55.572 17.3023 55.916C17.3023 56.156 17.3863 56.352 17.5543 56.504C17.7303 56.656 17.9503 56.776 18.2143 56.864C18.4783 56.952 18.8383 57.052 19.2943 57.164C19.9183 57.308 20.4303 57.46 20.8303 57.62C21.2303 57.772 21.5703 58.008 21.8503 58.328C22.1383 58.648 22.2823 59.08 22.2823 59.624C22.2823 60.088 22.1543 60.508 21.8983 60.884C21.6503 61.26 21.2703 61.56 20.7583 61.784C20.2463 62.008 19.6223 62.12 18.8863 62.12ZM31.5452 62L31.5332 56.432L28.7732 61.04H28.0772L25.3172 56.504V62H23.8292V53.6H25.1132L28.4492 59.168L31.7252 53.6H33.0092L33.0212 62H31.5452ZM40.7381 60.056H36.5381L35.7101 62H34.1021L37.8821 53.6H39.4181L43.2101 62H41.5781L40.7381 60.056ZM40.2221 58.832L38.6381 55.16L37.0661 58.832H40.2221ZM49.7744 62L48.0584 59.54C47.9864 59.548 47.8784 59.552 47.7344 59.552H45.8384V62H44.2784V53.6H47.7344C48.4624 53.6 49.0944 53.72 49.6304 53.96C50.1744 54.2 50.5904 54.544 50.8784 54.992C51.1664 55.44 51.3104 55.972 51.3104 56.588C51.3104 57.22 51.1544 57.764 50.8424 58.22C50.5384 58.676 50.0984 59.016 49.5224 59.24L51.4544 62H49.7744ZM49.7384 56.588C49.7384 56.052 49.5624 55.64 49.2104 55.352C48.8584 55.064 48.3424 54.92 47.6624 54.92H45.8384V58.268H47.6624C48.3424 58.268 48.8584 58.124 49.2104 57.836C49.5624 57.54 49.7384 57.124 49.7384 56.588ZM54.6425 54.92H51.8585V53.6H58.9865V54.92H56.2025V62H54.6425V54.92ZM66.4573 60.692V62H60.1573V53.6H66.2893V54.908H61.7173V57.092H65.7733V58.376H61.7173V60.692H66.4573ZM73.6924 62L71.9764 59.54C71.9044 59.548 71.7964 59.552 71.6524 59.552H69.7564V62H68.1964V53.6H71.6524C72.3804 53.6 73.0124 53.72 73.5484 53.96C74.0924 54.2 74.5084 54.544 74.7964 54.992C75.0844 55.44 75.2284 55.972 75.2284 56.588C75.2284 57.22 75.0724 57.764 74.7604 58.22C74.4564 58.676 74.0164 59.016 73.4404 59.24L75.3724 62H73.6924ZM73.6564 56.588C73.6564 56.052 73.4804 55.64 73.1284 55.352C72.7764 55.064 72.2604 54.92 71.5804 54.92H69.7564V58.268H71.5804C72.2604 58.268 72.7764 58.124 73.1284 57.836C73.4804 57.54 73.6564 57.124 73.6564 56.588Z" fill="#354D43"></path>
  </svg>
);

export default function InteractiveStats() {
  const containerRef = useRef<HTMLDivElement>(null);
  const cursorRef = useRef<HTMLDivElement>(null);

  useGSAP(() => {
    const cursor = cursorRef.current;
    const section = containerRef.current;
    if (!cursor || !section) return;

    // Check if the device supports hover (desktop)
    const isHoverSupported = window.matchMedia("(hover: hover)").matches;
    if (!isHoverSupported) return;

    // Initialize custom cursor positioning to center it on the mouse
    gsap.set(cursor, { xPercent: -50, yPercent: -50 });

    const xTo = gsap.quickTo(cursor, "x", { duration: 0.15, ease: "power3" });
    const yTo = gsap.quickTo(cursor, "y", { duration: 0.15, ease: "power3" });

    const moveCursor = (e: MouseEvent) => {
      xTo(e.clientX);
      yTo(e.clientY);
    };

    const handleMouseEnter = (e: MouseEvent) => {
      // Instantly position the cursor to the mouse pointer before scaling up
      gsap.set(cursor, { x: e.clientX, y: e.clientY });
      gsap.to(cursor, { scale: 1, opacity: 1, duration: 0.3, ease: "back.out(1.7)" });
    };

    const handleMouseLeave = () => {
      gsap.to(cursor, { scale: 0, opacity: 0, duration: 0.3, ease: "power2.in" });
    };

    // Attach cursor events to each stat item instead of the section
    const statItems = section.querySelectorAll(".interactive-stat");

    statItems.forEach((item) => {
      item.addEventListener("mousemove", moveCursor as EventListener);
      item.addEventListener("mouseenter", handleMouseEnter as EventListener);
      item.addEventListener("mouseleave", handleMouseLeave as EventListener);
    });
    
    return () => {
      statItems.forEach((item) => {
        item.removeEventListener("mousemove", moveCursor as EventListener);
        item.removeEventListener("mouseenter", handleMouseEnter as EventListener);
        item.removeEventListener("mouseleave", handleMouseLeave as EventListener);
      });
    };
  }, { scope: containerRef });

  return (
    <section ref={containerRef} className="interactive-stats module home-page__interactive-stats">
      
      {/* Custom Hover Cursor */}
      <div 
        ref={cursorRef} 
        className="fixed top-0 left-0 w-[120px] h-[120px] pointer-events-none z-[100] opacity-0 scale-0"
      >
        <Image src="/images/cursor-white.svg" alt="Build Smarter Cursor" fill className="object-contain" />
      </div>

      <div className="interactive-stats__container w-full flex flex-col">
        {STATS_DATA.map((item, idx) => (
          <InteractiveStatItem key={idx} item={item} />
        ))}
      </div>
    </section>
  );
}

function InteractiveStatItem({ item }: { item: StatBlock }) {
  const itemRef = useRef<HTMLAnchorElement>(null);
  const overContentRef = useRef<HTMLDivElement>(null);
  const overImageRef = useRef<HTMLDivElement>(null);

  useGSAP(() => {
    const content = overContentRef.current;
    const overImg = overImageRef.current;
    const itemEl = itemRef.current;
    if (!content || !itemEl) return;

    const mm = gsap.matchMedia();

    // Mobile scroll animation: simple scrub clip reveal
    mm.add("(max-width: 767px)", () => {
      gsap.fromTo(
        content,
        { clipPath: "inset(0% 100% 0% 0%)" },
        {
          clipPath: "inset(0% 0% 0% 0%)",
          scrollTrigger: {
            trigger: itemEl,
            start: "top bottom",
            end: "top 10%",
            scrub: 1,
          }
        }
      );
    });

    // Desktop hover + scroll animations
    mm.add("(min-width: 768px)", () => {
      // 1. Scroll-trigger reveal when entering viewport
      gsap.fromTo(
        content,
        { clipPath: "inset(0% 100% 0% 0%)" },
        {
          clipPath: "inset(0% 0% 0% 0%)",
          ease: "power2.inOut",
          duration: 0.6,
          scrollTrigger: {
            trigger: itemEl,
            start: "top 80%",
            toggleActions: "play none none reverse",
          }
        }
      );

      // Slide image background subtly on scroll entry
      if (overImg) {
        gsap.fromTo(
          overImg,
          { xPercent: -20 },
          {
            xPercent: 0,
            ease: "power1.out",
            duration: 1,
            scrollTrigger: {
              trigger: itemEl,
              start: "top 80%",
              toggleActions: "play none none reverse",
            }
          }
        );
      }
    });

    return () => mm.revert();
  }, { scope: itemRef });

  const handleMouseEnter = () => {
    if (typeof window !== "undefined" && !window.matchMedia("(hover: hover)").matches) return;
    if (!overContentRef.current || !overImageRef.current) return;
    // Scale up over content on hover
    gsap.timeline()
      .to(overContentRef.current, {
        clipPath: "inset(0% 0% 0% 0%)",
        ease: "power3.out",
        duration: 0.5,
        overwrite: "auto"
      })
      .to(overImageRef.current, {
        xPercent: 0,
        ease: "power1.out",
        duration: 0.8,
        overwrite: "auto"
      }, "<");
  };

  const handleMouseLeave = () => {
    if (typeof window !== "undefined" && !window.matchMedia("(hover: hover)").matches) return;
    if (!overContentRef.current || !overImageRef.current) return;
    // Slide out over content back to the left (right to left)
    gsap.timeline()
      .to(overContentRef.current, {
        clipPath: "inset(0% 100% 0% 0%)",
        ease: "power3.inOut",
        duration: 0.5,
        overwrite: "auto"
      })
      .to(overImageRef.current, {
        xPercent: -20,
        ease: "power1.inOut",
        duration: 0.5,
        overwrite: "auto"
      }, "<");
  };

  return (
    <Link 
      href={item.link} 
      ref={itemRef}
      className="interactive-stat interactive-stats__interactive-stat"
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      {/* UNDER CONTENT LAYER */}
      <div className="interactive-stat__content interactive-stat__content--under">
        <div className="interactive-stat__text-holder">
          <p className="interactive-stat__stat">{item.stat}</p>
          <p className="interactive-stat__explainer">{item.explainer}</p>
          <ArrowIcon />
        </div>
        <div className="asset interactive-stat__asset asset--standard">
          <picture className="asset__image-holder interactive-stat__image-holder interactive-stat__image-holder--desktop">
            <Image 
              src={item.imageSrcDesktop} 
              alt={item.explainer}
              fill
              className="asset__image interactive-stat__image"
              sizes="100vw"
            />
          </picture>
          <picture className="asset__image-holder interactive-stat__image-holder interactive-stat__image-holder--mobile">
            <Image 
              src={item.imageSrcMobile} 
              alt={item.explainer}
              fill
              className="asset__image interactive-stat__image"
              sizes="100vw"
            />
          </picture>
        </div>
      </div>

      {/* OVER CONTENT LAYER */}
      <div 
        ref={overContentRef}
        className="interactive-stat__content interactive-stat__content--over"
      >
        <div className="interactive-stat__text-holder">
          <p className="interactive-stat__stat">{item.stat}</p>
          <p className="interactive-stat__explainer">{item.explainer}</p>
          <ArrowIcon />
        </div>
        <div className="asset interactive-stat__asset asset--standard">
          <picture className="asset__image-holder interactive-stat__image-holder interactive-stat__image-holder--desktop">
            <div ref={overImageRef} className="w-full h-full relative">
              <Image 
                src={item.imageSrcDesktop} 
                alt={item.explainer}
                fill
                className="asset__image interactive-stat__image"
                sizes="100vw"
              />
            </div>
          </picture>
          <picture className="asset__image-holder interactive-stat__image-holder interactive-stat__image-holder--mobile">
            <div className="w-full h-full relative">
              <Image 
                src={item.imageSrcMobile} 
                alt={item.explainer}
                fill
                className="asset__image interactive-stat__image"
                sizes="100vw"
              />
            </div>
          </picture>
        </div>
      </div>
    </Link>
  );
}
